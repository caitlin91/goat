...
##	Style coding by Meredith Tamminga, 2013
## 	Style coding removed (with following word left in) by Caitlin Halfacre, 2019
###########################################################
...
form Search Settings
	...
	sentence Search_Pre_Context N NG R L M AA AE AH AO AW AX AY EH ER EY IH IY OW OY UH UW
	sentence Search_Post_Context AA AE AH AO AW AX AY EH ER EY IH IY OW OY UH UW
	...
	sentence Stop_Words UNTIL SOMETIMES CURRENTLY APPARENTLY DEFINITELY ABSOLUTELY LITERALLY
	comment Extra Settings
	positive Window_Size 3
	sentence Default_Code1 4
	sentence Default_Code2
	real Start_Time 0 
	boolean Play_on_Continue 1
endform
...
if start_Time == 0
	fileappend 'outfile$'	File	Segment	Position	Code1	Code2	Seg_Start	Seg_End	Word	Word_Start	Word_End	Pre_Seg	Pre_Seg_Start	Pre_Seg_End	Post_Seg	Post_Seg_Start	Post_Seg_End	Post_Word Window	Vowels_per_Second	Comments	Warnings'newline$'
endif
		...
		for int from start_Interval to word_Intervals
			...
			if int < word_Intervals
				post_Word$ = Get label of interval... 2 int+1
			else
				post_Word$ = "none"
			endif
					...
					if condition_met = 0
						...
						if condition_met == 1
							...
							beginPause ("Code it")
								...
								boolean("Reject", 0)
							#	Play window
							click = endPause ("Play", "Continue", 2)
							if click == 1
								Play window
							else
								coded = 1
							...
							if reject ==0
								fileappend 'outfile$' 'file$'	'seg$'	'location$'	'code1$'	'code2$'	'seg_Start:3'	'seg_End:3'	'word$'	'word_Start:3'	'word_End:3'	'pre_Seg$'	'pre_Seg_Start:3'	'pre_Seg_End:3'	'post_Seg$'	'post_Seg_Start:3'	'post_Seg_End:3'	'post_Word$' 'real_Dur:3'	'vowels_Per_Second:3'	'comment$'	'warning$''newline$'
							endif
						endif		
					endif
				endfor
